 ...
 */
   ...
   ////Bind Variables
   //Set Campus Area that we need to look for.
...
{ //var_dump($row["residence_group"]);        
        //var_dump($row);
        $residence=$row["residence_group"];
}  
    ...
    if($rowCount>=1){
       ...
       //Trim characters so that we can just focus on the first two digits of the number.
        ...
        //$campusarea
        //echo "<a href='http://localhost/apps/checkin/report_for_campus_area.php?residence=".$residenceLOCATION."&dateNEEDED=".$date_needed."&beginTIME=".$timeBEGINforReport."&&endTIME=".$timeENDforReport."' target='_blank')'>$rowCountArray[0]</a>";
        echo "<a href='http://localhost/apps/checkin/report_for_campus_area.php?central_location=".$campusarea."&dateNEEDED=".$date_needed."&beginTIME=".$timeBEGINforReport."&&endTIME=".$timeENDforReport."' target='_blank')'>$rowCountArray[0]</a>";
