 ...
 */
   ...
   $sql=$group1_RESIDENCE->getSQLProvideBuilding_ProvideDateRange();
   ...
   $statement->execute();   
    ...
    if($rowCount>=1){
       ...
       //Trim characters so that we can just focus on the first two digits of the number.
        ...
        echo "<a href='http://localhost/apps/checkin/report_for_all_dates.php?residence=".$residenceLOCATION."&dateNEEDEDSTART=".$beginDATENEEDED."&dateNEEDEDEND=".$endDATENEEDED."&beginTIME=".$timeBEGINforReport."&&endTIME=".$timeENDforReport."' target='_blank')'>$rowCountArray[0]</a>";
