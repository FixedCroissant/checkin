<?php
/* Author: J.Williams
 * Title: completeTOTALSforGraph.php
 * Description:
 *
 *
 */
   ...
   $statement->execute();   
    ...
    //Updated the totals for the week of Monday June 15 - Sunday June 21 (Date of Summer Session II checkin).

    //Display the results of everything.
    while($row=$statement->fetch(PDO::FETCH_ASSOC)){        
        //Get Monday's Information
        if($row["date_of_swipe"]==="2015-06-15"){
        ...
        if($row["date_of_swipe"]==="2015-06-20"){
            $countFORSATURDAY++;
                    $saturdayINFO[]=$row["residence_group"];
        }
        
        //Get Sunday's Information
        if($row["date_of_swipe"]==="2015-06-21"){
            $countFORSUNDAY++;
